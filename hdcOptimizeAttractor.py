...
from parametersHDC import lam_hdc, n_hdc

import time

# returns the stable activity peak
def getActivityPeak(n, lam):
    connectivity = HDCAttractorConnectivity(n, lam)
    topo = NetworkTopology()
    hdcNetwork.addHDCAttractor(topo, n, connectivity.connection)
    inst = topo.makeInstance()
    hdcNetwork.initializeHDC(inst, 0.0)
...
def plotActivityPeaks(n=n_hdc, lams=[500, 21000, 35000]):
    ...
    target = [targetPeakDefault(x) for x in X]
    results = []
    for lam in lams:
        results.append(getActivityPeak(n, lam))
    # plot 
...
def plotWeightFunction(n=n_hdc, lam=lam_hdc):
    connectivity = HDCAttractorConnectivity(n, lam)
    X = range(50)
    Y = [connectivity.connection(0, x) for x in X]
